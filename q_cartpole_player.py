...
# Lecture 12, 29 May 2018.
# Erik Spence.
#
# This file, q_cartpole_player.py, contains code used for lecture 12.
...
#        print loss
    ...
    def rewards_frac(self):

        # This function calculates the fraction of moves in the data
        # set which generate positive results.
        
        rewards = [r[2] for r in self.observations]
        return sum(rewards) / float(len(self.observations))

    
    ####################################

    
    def remove_bad_point(self):

        # This function removes a point from data set which does not
        # generate a positive result.
        stop = False
        i = 0
        while (not stop):
            if self.observations[i][2] == 0:
                remove = self.observations.pop(i)
                stop = True
            else:
                i += 1
